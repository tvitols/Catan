...
Board::Board() {
    ...
    //Sets Robber on the desert tile
...
void Board::generateTiles(std::mt19937 twist) {
    ...
    std::vector<Vertex*> v;
    //Types of available port Resources
    ...
    //Vertex indices that need do be Ports
    ...
    //Setting coordinates for all Tiles, Edges, and Vertices
...
Edge * Board::getEdge(coords coordinates) {
    //iterating through Tiles to check if the coordinates correspond to an Edge
    for (int i = 0; i < tiles.size(); i++) {
        for (int j = 0; j < tiles[i].size(); j++) {
...
Vertex * Board::getVertex(coords coordinates) {
    //iterating through Tiles to check if the coordinates correspond to a Vertex
    for (int i = 0; i < tiles.size(); i++) {
        for (int j = 0; j < tiles[i].size(); j++) {
...
Edge * Board::getEdge(const std::string &message) {
    //getEdge but first printing Board with a specific message
    coords xy = printBoard(message);
    return getEdge(xy);
}

Vertex * Board::getVertex(const std::string &message) {
    //getVertex but first printing the Board with a specific message
    return getVertex(printBoard(message));
}


Tile* Board::getTile(coords coordinates) {
    //iterating through Tiles to check if the coordinates correspond to a Tile
    for (int i = 0; i < tiles.size(); i++) {
        for (int j = 0; j < tiles[i].size(); j++) {
